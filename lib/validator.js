...
_.assign(Validator.prototype, {
	...
	validate: function (_data, options) {
		var data = _.cloneDeep(_data);

		var result = this._validate(data, this.schema, this.getOptions(this.schema, options));
	...
	valid: function (val, data, property, options) {
		...
		if (_.isUndefined(val)) {
			if (property.required && !options.skipRequired) {
	...
	getOptions: function (schema, options) {
		return _.assign(_.pick(schema, ['mode', 'cast']), options);
	}
