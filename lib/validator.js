...
var defaultTypes = require('./rules/types');
...
function Validator(schema, options) {
	...
	this.lexicon = require(this.options.langFile);

	this.types = defaultTypes;
	this.build(schema);
	this.validation = new Validation({
		lexicon: this.lexicon,
		customRules: {}			// todo: add ability to pass in custom rules
	});
}
...
_.assign(Validator.prototype, {
	build: function (schema) {
		...
		var buildProperty = function(prop, key){
			if(prop.validation){
	...
	_validate: function (data, schema, options, _fullKey, parent) {
		...
		_.each(schema.properties, function (property, key) {
			...
			if (!validated.success) {
				errors[propertyFullKey] = validated.error ||  this.getErrorMessage('VALIDATION_ERROR_MISSING');
			}
